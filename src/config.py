...
# DEFAULT_ROI_FILE = "/home/atp/projects/change_detection/roi.json"
...
# ── Paths (project-root relative) ─────────────────────────────────────────────
PROJECT_ROOT = Path(__file__).resolve().parents[1]  # .../change_detection
MODELS_DIR = PROJECT_ROOT / "weights"
...
class DetectorConfig:
    ...
    # Matching / thresholds
    # ── CHANGED: lowered from 0.65 → 0.50 ──────────────────────────────────
    # DINO is the sole arbiter of whether two objects at approximate
    # coordinates match.  YOLO class names are NOT used at all.
    similarity_threshold: float = 0.5
    double_check_threshold: float = 0.60
    # ── CHANGED: increased from 50 → 100 ────────────────────────────────────
    # Wider search radius so that objects with slight positional shift
    # are still considered "nearby" and matched via DINO directly,
    # rather than falling through to unnecessary double-checks.
    position_tolerance_px: int = 100
    alpha: float = 0.25  # kept for backward compat (no longer used in matching)
